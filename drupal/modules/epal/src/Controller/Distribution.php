...
class Distribution extends ControllerBase
{
                ...
                if ($this->choice_id === 1) {
                      ...
                      $sCon->join('epal_student_epal_chosen', 'epals', "eStudent.id = epals.student_id AND epals.choice_no = {$this->choice_id}");
            	        ...
            	        $num = 1;
            					$j = 1;
                      while ($num <= $numData) {
                        if (($total_located = $this->locateStudent($this->choice_id, self::IS_FIRST_PERIOD, null, 1 + $sizeOfBlock * ($j-1), $j * $sizeOfBlock, null)) === self::ERROR_DB) {
				                      $transaction->rollback();
                            return $this->respondWithStatus([
                                "message" => t("Unexpected Error")
                            ], Response::HTTP_INTERNAL_SERVER_ERROR);
                    ...
                    if (sizeof($this->pendingStudents) != 0) {
                        if ($this->locateStudent($this->choice_id, self::IS_FIRST_PERIOD, null, null, null, $this->pendingStudents) === self::ERROR_DB) {
				            $transaction->rollback();
                            return $this->respondWithStatus([
                                "message" => t("Unexpected Error")
                            ], Response::HTTP_INTERNAL_SERVER_ERROR);
     ...
     * @param string datelimit (in "d-m-y" format) is the start date that second period distribution uses for student applications date
     ...
     */
    public function locateStudent($choice_id, $period = false, $datelimit = null, $id_range_start = null, $id_range_end = null, $id_list = null)
    {
        $total_count = 0;
        try {
            $sCon = $this->connection->select('epal_student', 'eStudent')
    ...
    public function locateSecondPeriodStudents(Request $request, $dateStart)
    {
        set_time_limit(600); // dev
        ...
        try {
            ...
            if ($this->initializeResultsInSecondPeriod($dateStart) === self::ERROR_DB) {
      				$transaction->rollback();
      				return $this->respondWithStatus([
      					"message" => t("Unexpected Error")
      				], Response::HTTP_INTERNAL_SERVER_ERROR);
            ...
            if ($this->locateStudent(1, self::IS_SECOND_PERIOD, $dateStart) === self::ERROR_DB) {
				       $transaction->rollback();
				       return $this->respondWithStatus([
					            "message" => t("Unexpected Error1"),
				               ], Response::HTTP_INTERNAL_SERVER_ERROR);
            }
    ...
    private function initializeResultsInSecondPeriod($datelimit)
    {
		// initialize/empty epal_student_class if there are already data in it!
