...
class OAuthLogout extends ControllerBase
{
    ...
    public function logoutGo(Request $request)
    {
        ...
        try {
            ...
            $applicantUsers = $this->entityTypeManager
                ->getStorage('applicant_users')
            ...
            if ($applicantUser) {
                $user = $this->entityTypeManager
                    ->getStorage('user')
                    ->load($applicantUser->user_id->target_id);
                /*
                $userss = $this->entityTypeManager
                    ->getStorage('user')
                    ->loadByProperties(array('uid' => $applicantUser->user_id->target_id));
                $user = reset($userss);
                */

                $logout_token = $applicantUser->getRequesttoken();
                if ($user) {
                    $res = \Drupal::httpClient()->get($this->logout_url . $username, array('headers' => array('Accept' => 'text/plain')));
                    $user->setPassword(uniqid('pw'));
                    $user->save();
                    $applicantUser->set('accesstoken', '-');
                    $applicantUser->set('accesstoken_secret', '-');
                    $applicantUser->set('authtoken', '-');
                    $applicantUser->set('requesttoken', '-');
                    $applicantUser->set('requesttoken_secret', '-');
                    $applicantUser->save();
                    $foundUser = true;
                }
            }
