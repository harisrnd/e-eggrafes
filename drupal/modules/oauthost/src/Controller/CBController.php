...
class CBController extends ControllerBase
{
    ...
    public function loginCB(Request $request)
    {
        ...
        if ($ostauthConfig) {
            ...
            $this->default_role = $ostauthConfig->default_role->value;
        } else {
            $response = new Response();
        ...
        if ($schoolToken) {
            ...
            $response = new Response();
            $response->setContent('forbidden: schoolToken false');
            $response->setStatusCode(Response::HTTP_FORBIDDEN);
            $response->headers->set('Content-Type', 'application/json');
            return $response;
        }
    ...
    public function authenticatePhase2($request, $authToken, $authVerifier)
    {
        try {
            ...
            $schoolToken = md5(uniqid(mt_rand(), true));

            if ($schoolUser) {
                $user = $this->entityTypeManager->getStorage('user')->load($schoolUser->user_id->target_id);
                if ($user) {
                    ...
                    $schoolUser->set('requesttoken',$this->requestToken);
                    $schoolUser->set('requesttoken_secret', $this->requestTokenSecret);
                    $schoolUser->set('timelogin', $currentTime);
                    $schoolUser->set('userip', $request->getClientIp());

                    $schoolUser->save();
                }
            }
            if ($schoolUser === null || !$schoolUser) {
                //Create a User
                $user = User::create();
                //Mandatory settings
                $unique_id = uniqid('####');
                ...
                $user->setUsername($schoolToken); //This username must be unique and accept only a-Z,0-9, - _ @ .
                $user->activate();
                ...
                if ($user) {
                    ...
                    try  {
                      ...
                      unset($crypt);
                      $this->logger->notice('schoolToken encoding false');
                      $response = new Response();
                      $response->setContent('internal_server_error');
                      $response->setStatusCode(Response::HTTP_INTERNAL_SERVER_ERROR);
                      $response->headers->set('Content-Type', 'application/json');
            ...
            $this->oauthostSession->set('authtoken', $schoolToken);
